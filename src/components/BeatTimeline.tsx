...
export function BeatTimeline() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (activeData?.type === 'reference-frame') {
      ...
      // PRIORITY 1: Check if dropped on a shot drop zone (highest priority - this is an existing shot)
      ...
      // PRIORITY 2: Check if dropped on shot-drop- prefix
      ...
      // PRIORITY 3: Check if dropped directly on a shot (the sortable ID)
      for (const beat of beats) {
      ...
      // PRIORITY 4: Check if dropped on beat section (between shots or empty space) - creates new shot at end
      ...
      // PRIORITY 5: Check if dropped on gap zone by ID - ONLY if NOT on a shot
      if (typeof over.id === 'string' && over.id.startsWith('gap-')) {
        const parts = over.id.split('-');
        if (parts.length >= 3) {
          const beatId = parts[1] + (parts[2] ? '-' + parts[2] : '');
          const insertAfterIndex = parseInt(parts[parts.length - 1], 10);
          if (!isNaN(insertAfterIndex)) {
            createShotFromFrame(frameIndex, fromShotId, beatId, insertAfterIndex);
            return;
          }
        }
      }
      
      // PRIORITY 6: Check if dropped on beat section by ID
    ...
    // Check if over is a beat section
    const overBeat = beats.find((b) => b.beat_id === over.id);
    if (overBeat) {
